...
int execute_elf(const unsigned char *file)
{
	...
	DEBUG_MSG("Reading program header..")
	read_program_header(data, file);

    DEBUG_MSG("Reading section header..")
	read_section_header(data, file);
	DEBUG_MSG("Reading string table..")
	read_string_table(data, file);
	
	DEBUG_MSG("Reading symbol table..")
	read_symbol_table(data, file);
	DEBUG_MSG("Reading symbol string table..")
	read_symbol_string_table(data, file);

	DEBUG_MSG("Reading text section..")
	read_text_section(data, file);
	
	DEBUG_MSG("Reading bss section..")
